...
def main():
    ...
    # test for non-int coursenum
    request = '/regoverviews?dept=COS&coursenum=abc&area=qr&title=intro'
    run_test(serverurl, request)

    request = '/regoverviews?dept=COS&coursenum=abc&area=qr&title=C%S'
    run_test(serverurl, request)

    request = '/regoverviews?dept=COS&coursenum=abc&area=qr&title=C_S'
    run_test(serverurl, request)

    request = '/regdetails?classid=8321'
    run_test(serverurl, request)

    # test for non-existing classid
